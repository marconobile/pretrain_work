...
from source.utils.mol_utils import drop_disconnected_components, get_dihedral_angles, get_rdkit_conformer, minimize_energy, smi_reader_params
...
def getSettings(max_num_out_confs:int = 100, minRMSD:float=2.5):
    ...
    settings.settings.minRMSD = minRMSD # 1.3 # for freesolv test #! this is core ; 2.5
    settings.settings.energyWindow = 150000.0
    settings.settings.setMaxNumOutputConformers(max_num_out_confs) # mostly irrelevant?
    return settings
...
#     conf_gen.settings.setSamplingMode(2) # AUTO = 0; SYSTEMATIC = 1; STOCHASTIC = 2;
#     conf_gen.settings.timeout = max_time
#     conf_gen.settings.minRMSD = min_rmsd
#     conf_gen.settings.energyWindow = e_window
#     conf_gen.settings.setMaxNumOutputConformers(100)
#     # conf_gen.settings.maxNumOutputConformers = 100 # apply the -n argument
#     return conf_gen
...
def generate_conformers(smi:str, conf_gen:ConfGen.ConformerGenerator)-> list:
...
def load_conformers_from_sdf(mol_ensamble, keep_n:int) -> list:
